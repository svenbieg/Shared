...
Handle<Buffer> Base64Decode(LPCSTR input, UINT len)
{
if(!input)
	return nullptr;
UINT size=Base64Decode(input, len, nullptr, 0);
if(!size)
	return nullptr;
...
Handle<Buffer> Base64Decode(LPCWSTR input, UINT len)
{
if(!input)
	return nullptr;
if(!len)
...
StringCopy(str, len+1, input);
auto buf=Base64Decode(str, len);
delete str;
return buf;
}

Handle<String> Base64DecodeString(LPCSTR input, UINT len)
{
auto buf=Base64Decode(input, len);
len=buf->GetSize();
Handle<String> str=new String(len, nullptr);
auto str_ptr=const_cast<LPTSTR>(str->Begin());
StringCopy(str_ptr, len+1, (LPCSTR)buf->Begin(), len);
return str;
}
...
for(; pos<input_size; pos++)
	...
	if(!input[pos])
		break;
	BYTE offset=(BYTE)input[pos];
	if(Base64Table[offset]!=255)
		size++;
	}
if(input_size==-1)
	input_size=pos;
