...
return CONFIG_CLUSTER_GROUP_SIZE;
}
...
while(before>=0||after<child_count)
	...
	if(before>=0)
		...
		if(count<CONFIG_CLUSTER_GROUP_SIZE)
			return before;
		before--;
		}
...
void parent_group_remove_group(multi_heap_handle_t heap, parent_group_t* group, uint16_t at)
{
uint16_t child_count=cluster_group_get_child_count((cluster_group_t*)group);
MULTI_HEAP_ASSERT(at<child_count);
...
multi_heap_free_to_cache(heap, child);
}
