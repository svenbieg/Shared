...
bool block_map_group_add_block(multi_heap_handle_t heap, block_map_group_t* group, heap_block_info_t const* info, bool again)
{
...
bool block_map_group_get_block(multi_heap_handle_t heap, block_map_group_t* group, size_t min_size, heap_block_info_t* info)
{
...
bool block_map_group_remove_block(multi_heap_handle_t heap, block_map_group_t* group, heap_block_info_t const* info)
{
if(cluster_group_get_level(group)==0)
...
block_map_item_group_t* block_map_item_group_create(multi_heap_handle_t heap)
{
block_map_item_group_t* group=(block_map_item_group_t*)multi_heap_alloc_internal(heap, sizeof(block_map_item_group_t));
if(group==NULL)
	return NULL;
...
{
MULTI_HEAP_ASSERT(at<cluster_group_get_child_count((cluster_group_t*)group));
...
bool block_map_item_group_add_block(multi_heap_handle_t heap, block_map_item_group_t* group, heap_block_info_t const* info)
{
bool exists=false;
...
bool block_map_item_group_get_block(multi_heap_handle_t heap, block_map_item_group_t* group, size_t min_size, heap_block_info_t* info, bool passive)
{
uint16_t child_count=cluster_group_get_child_count((cluster_group_t*)group);
bool exists=false;
...
if(pos==child_count)
	return false;
block_map_item_t* item=block_map_item_group_get_item_at(group, pos);
MULTI_HEAP_ASSERT(item->offset!=0);
info->size=item->size;
if(item->index)
	{
	offset_index_t index;
...
bool block_map_item_group_remove_block(multi_heap_handle_t heap, block_map_item_group_t* group, heap_block_info_t const* info)
{
bool exists=false;
uint16_t pos=block_map_item_group_get_item_pos(group, info->size, &exists);
MULTI_HEAP_ASSERT(exists);
...
if(item->index)
	...
	{
	MULTI_HEAP_ASSERT(item->offset==info->offset);
	block_map_item_group_remove_item_at(group, pos, false);
	}
return true;
}
...
block_map_parent_group_t* block_map_parent_group_create(multi_heap_handle_t heap, uint16_t level)
{
block_map_parent_group_t* group=(block_map_parent_group_t*)multi_heap_alloc_internal(heap, sizeof(block_map_parent_group_t));
if(group==NULL)
	return NULL;
...
block_map_parent_group_t* block_map_parent_group_create_with_child(multi_heap_handle_t heap, block_map_group_t* child)
{
block_map_parent_group_t* group=(block_map_parent_group_t*)multi_heap_alloc_internal(heap, sizeof(block_map_parent_group_t));
if(group==NULL)
	return NULL;
...
bool block_map_parent_group_add_block(multi_heap_handle_t heap, block_map_parent_group_t* group, heap_block_info_t const* info, bool again)
{
...
bool block_map_parent_group_add_block_internal(multi_heap_handle_t heap, block_map_parent_group_t* group, heap_block_info_t const* info, bool again)
{
uint16_t pos=0;
...
bool block_map_parent_group_combine_child(multi_heap_handle_t heap, block_map_parent_group_t* group, uint16_t pos)
{
...
bool block_map_parent_group_combine_children(multi_heap_handle_t heap, block_map_parent_group_t* group)
{
bool combined=false;
uint16_t child_count=cluster_group_get_child_count((cluster_group_t*)group);
for(uint16_t pos=0; pos<child_count; )
...
bool block_map_parent_group_get_block(multi_heap_handle_t heap, block_map_parent_group_t* group, size_t min_size, heap_block_info_t* info, bool passive)
{
uint16_t pos=0;
...
bool block_map_parent_group_remove_block(multi_heap_handle_t heap, block_map_parent_group_t* group, heap_block_info_t const* info)
{
uint16_t pos=0;
...
bool block_map_parent_group_split_child(multi_heap_handle_t heap, block_map_parent_group_t* group, uint16_t at)
{
uint16_t child_count=cluster_group_get_child_count((cluster_group_t*)group);
if(child_count==CONFIG_CLUSTER_GROUP_SIZE)
	return false;
